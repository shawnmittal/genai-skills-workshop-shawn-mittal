...
def get_lat_lon_from_address(address: str, api_key: str) -> Optional[Tuple[float, float]]:
...
def get_weather_forecast(latitude: float, longitude: float) -> Optional[List[Dict[str, Any]]]:
    ...
    This function takes a latitude and longitude, first finds the corresponding
    NWS grid forecast endpoint, and then fetches the detailed weather forecast
    for that location.
    ...
    try:
        ...
        if not forecast_url:
            print("Error: Could not find the forecast URL in the API response.")
            return None
